...
* I/O Redirection 은 명령 행의 방향 지정이라고 할수있다 
...
## 쉘 스크립트(Shell Script) 
* 명령어들이 나열되어 있는 파일
* 쉘은 이 파일을 읽어서 마치 커맨드라인에 직접 명령어를 입력해 실행하는 것처럼 수행한다 

### 쉘(Shell)
* 도스상에서 내리는 대부분의 명령은 command.com 이라는 파일이 처리하는데 이를 운영체제 쉘(Shell) 프로그램이라고 한다 
* 쉘 프로그램은 사용자가 내린 명령을 운영체제가 수행할수 있도록 명령을 입력받고 전달하는 일을 한다 
* 사용자가 내린 명령을 운영체제가 수행할수 있도록 통역하는 역활을 한다 

### 스크립트(Script)
* 다른 프로그램에 의해 번역되거나 수행되는 프로그램이나 명령어들의 나열을 말한다 
* 스크립트 언어는 소스 코드를 컴파일하지 않고 실행할수있는 프로그래밍 언어이다 
...
[출처](https://daeunginfo.blogspot.com/2019/04/linux.html)

10-04-20

## 디렉토리 
* 리눅스 파일 체제의 최상의 디렉토리를 루트(/) 라고 표현  
* 절대경로 : 파일 시스템 전체를 기준으로 파일이나 디렉토리의 절대적인 위치를 기준으로 시작하는 경로 
* 상대경로 : 현재 작업하고 있는 디렉토리에서 파일이나 디렉토리의 상대적인 위치를 말한다 
    * ./ -> 현재 경로
    * ../ -> 현재 경로 위 단계  
* 최상위 / 로 시작하기 때문에 트리구조 라고 한다   

### 특징 
* /bin : 기본적인 명령어가 저장 
* /boot : 리눅스 부트로더가 존재 
* /dev : 시스템 device 파일 저장  
* /etc : 대부분의 모든 시스템 관련 설정파일 존재 
* /home, /root : 일반 사용자, 루트사용자를 위한 홈 디렉토리
* /lib : 커널 모듈 파일과 라이브러리파일 존재 
* /media : 외부입력장치에 관한 마운트포인트 디렉토리 
* /opt : 추가 소프트웨어 디렉토리 
* /sbin : 부팅, 복원, 복구 및 수리를 위한 중요한 명령어
* /srv : 시스템에 의해 제공되는 서비스에 대한 자료 
* /tmp : 임시파일 저장공간
* /usr : 시스템이 아닌 일반 사용자들이 주로 사용 
* /var : 가변 자료파일 저장
* /proc : 가상 파일 시스템 